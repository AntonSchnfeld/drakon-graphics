...
public final class ProbeTextureInitializationChecks {
    ...
    public static void run() {
        ...
        try (Arena arena = Arena.ofConfined()) {
            MemorySegment data = arena.allocate(20).asSlice(2, 16);
            ...
            expect(IllegalArgumentException.class,
                    () -> device.createTexture(sampled, data, ResourceState.COLOR_ATTACHMENT_WRITE));
            initialized = device.createTexture(sampled, data, ResourceState.SAMPLED_READ);
        }
        MemorySegment tooSmall = MemorySegment.ofArray(new byte[15]);
        expect(IllegalArgumentException.class, () -> device.createTexture(sampled, tooSmall, ResourceState.SAMPLED_READ));
        MemorySegment tooLarge = MemorySegment.ofArray(new byte[17]);
        ...
        expect(IllegalArgumentException.class, () -> device.createTexture(
                depth, MemorySegment.ofArray(new byte[16]), ResourceState.DEPTH_ATTACHMENT_WRITE));
